...
class BHDVCStf(object): 
    ...
    def total_xs(self, kins, cffs,phi):
        ...
        return f_pred
    @tf.function
    def total_xs2(self, kins, cffs, phi, ReH):
        ffs = FFs()
        #c0fit = dvcs 
        ...
        F1, F2 = ffs.F1_F2(t) # calculating F1 and F2 using passed data as opposed to passing in F1 and F2
        ReE, ReHtilde, c0fit = tf.split(cffs, num_or_size_splits=3, axis=1)  # output of network
        ee, y, xi, Gamma, tmin, Ktilde_10, K = self.SetKinematics(QQ, x, t, k)
        P1, P2 = self.BHLeptonPropagators(phi, QQ, x, t, ee, y, K)
        xsbhuu = self.BHUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, Gamma, K)
        xsiuu = self.IUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, K, Gamma, ReH, ReE, ReHtilde, "t2", tmin, xi, Ktilde_10)
        ...
        return f_pred
    @tf.function
    def total_xs3(self, kins, cffs, phi, ReH, ReE):
        ffs = FFs()
        #c0fit = dvcs 
        ...
        F1, F2 = ffs.F1_F2(t) # calculating F1 and F2 using passed data as opposed to passing in F1 and F2
        ReHtilde, c0fit = tf.split(cffs, num_or_size_splits=2, axis=1)  # output of network
        ee, y, xi, Gamma, tmin, Ktilde_10, K = self.SetKinematics(QQ, x, t, k)
        P1, P2 = self.BHLeptonPropagators(phi, QQ, x, t, ee, y, K)
        xsbhuu = self.BHUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, Gamma, K)
        xsiuu = self.IUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, K, Gamma, ReH, ReE, ReHtilde, "t2", tmin, xi, Ktilde_10)
        ...
        return f_pred 
    @tf.function
    def total_xs4(self, kins, cffs, phi, ReH, ReE, ReHtilde ):
        ffs = FFs()
        ...
        F1, F2 = ffs.F1_F2(t) # calculating F1 and F2 using passed data as opposed to passing in F1 and F2
        c0fit = tf.split(cffs, num_or_size_splits=1, axis=1)  # output of network
        ee, y, xi, Gamma, tmin, Ktilde_10, K = self.SetKinematics(QQ, x, t, k)
        P1, P2 = self.BHLeptonPropagators(phi, QQ, x, t, ee, y, K)
        xsbhuu = self.BHUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, Gamma, K)
        xsiuu = self.IUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, K, Gamma, ReH, ReE, ReHtilde, "t2", tmin, xi, Ktilde_10)
        ...
        return f_pred 
    @tf.function
    def total_xs5(self, kins, cffs,phi, dvcs):
        ffs = FFs()
        c0fit = dvcs
        ...
        return f_pred 
    @tf.function
    def total_xs6(self, kins, cffs,phi, dvcs, ReH):
        ffs = FFs()
        c0fit = dvcs
        ...
        return f_pred
    @tf.function
    def total_xs7(self, kins, cffs,phi, dvcs, ReH, ReE): 
        ffs = FFs()
        c0fit = dvcs
        ...
        F1, F2 = ffs.F1_F2(t) # calculating F1 and F2 using passed data as opposed to passing in F1 and F2
        ReHtilde = tf.split(cffs, num_or_size_splits=1, axis=1)  # output of network 
        ee, y, xi, Gamma, tmin, Ktilde_10, K = self.SetKinematics(QQ, x, t, k)
        P1, P2 = self.BHLeptonPropagators(phi, QQ, x, t, ee, y, K)
        xsbhuu = self.BHUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, Gamma, K)
        xsiuu = self.IUU(phi, F1, F2, P1, P2, QQ, x, t, ee, y, K, Gamma, ReH, ReE, ReHtilde, "t2", tmin, xi, Ktilde_10)
...
def gen_replica(pseudo):
    ...
    replica_dict = {'set': pseudo['set'], 
                    ...
                    'phi': pseudo['phi'], 'F': F_rep,'errF': errF_rep, 'ReH':pseudo['ReH'], 'ReE':pseudo['ReE'], 'ReHtilde':pseudo['ReHtilde'], 'dvcs':pseudo['dvcs']}       
    return pd.DataFrame(replica_dict)

# Reduced chi2 custom Loss function (model predicted inside loss)

        
def rchi2_Loss5(kin, pars, F_data, F_err, phi, dvcs): 
    #print('pars') 
    #print(pars[0])
    kin = tf.cast(kin, pars.dtype)
    F_dnn = tf.reshape(bkm10.total_xs5(kin, pars, phi, dvcs), [-1])
    F_data = tf.cast(F_data, pars.dtype)
    F_err = tf.cast(F_err, pars.dtype) 
    loss = tf.reduce_mean(tf.square( (F_dnn - F_data) / (F_err) ) ) 
    return loss 

def fit_replica5(i, pseudoo, uniqu_cff):
    epochs = 500
    # ----- 2 input data -----------  
    #pseduo = filter_unique_sets(pseudo)
    
    
    def build_model5():    #  smae as above, but longer as it got cut off, 5000 epochs
...
#think I am goona keep, train and test outputs the same, so just the value on first set of phi
    ...
    def train_step(loss_inputs, inputs, targets, weights):
        for ii in range(23, -1, -1):
            ...
            with tf.GradientTape() as tape:
                pars = model(inputs) 
                loss_value = rchi2_Loss5(loss_inputs, pars, targets[ii], weights[ii], PHI[ii], dvcss)
    ...
    def test_step(loss_inputs, inputs, targets, weights):
        pars = model(inputs)
        val_loss_value = rchi2_Loss5(loss_inputs, pars, targets, weights, PHI[0], dvcsvv)
...
###################################################################################################################################
        ...
        #kin3_norm = np.array([QQ_norm, xB_norm, t_norm]) 
            ...
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} ReH_mape: {:.5f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value,     pars_mape[0], pars_mape[1], pars_mape[2], total_mape)
            joined_array = np.concatenate((predictions_resultss, kin3_norm), axis=1)
           
            cff_results = pd.DataFrame(joined_array, columns = [ 'ReH_results', 'ReE_results', 'ReHt_results', 'QQ', 'x_B', 't' ])
            
            cff_results.to_csv('1okin_spreed3.csv', mode='a', index=False) 
            results_file_path = '1okin_spreed3.py'
...
def rchi2_Loss6(kin, pars, F_data, F_err, phi, dvcs, ReH):  
    #print('pars') 
    #print(pars[0])
    kin = tf.cast(kin, pars.dtype)
    F_dnn = tf.reshape(bkm10.total_xs6(kin, pars, phi, dvcs, ReH), [-1])
    F_data = tf.cast(F_data, pars.dtype)
    F_err = tf.cast(F_err, pars.dtype) 
    loss = tf.reduce_mean(tf.square( (F_dnn - F_data) / (F_err) ) ) 
    return loss 
...
#think I am goona keep, train and test outputs the same, so just the value on first set of phi
    ...
    def train_step(loss_inputs, inputs, targets, weights):
        for ii in range(23, -1, -1):
            ...
            with tf.GradientTape() as tape:
                pars = model(inputs) 
                loss_value = rchi2_Loss6(loss_inputs, pars, targets[ii], weights[ii], PHI[ii], dvcss, ReHss)
            grads = tape.gradient(loss_value, model.trainable_weights)
            optimizer.apply_gradients(zip(grads, model.trainable_weights))
        return loss_value
    ...
    def test_step(loss_inputs, inputs, targets, weights):
        pars = model(inputs)
        val_loss_value = rchi2_Loss6(loss_inputs, pars, targets, weights, PHI[0], dvcsvv, ReHvv)
        return val_loss_value
...
###################################################################################################################################
        if (epoch==(epochs-1)):
            predictionss = model.predict(kin3_norm)
            predictions_resultss = np.array(predictionss)
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, pars_mape[0], pars_mape[1], total_mape) 
            joined_array = np.concatenate((predictions_resultss, kin3_norm), axis=1)
           
            cff_results = pd.DataFrame(joined_array, columns = [ 'ReE_results', 'ReHt_results', 'QQ', 'x_B', 't' ])
           
            
            
            cff_results.to_csv('2okin_spreed3.csv', mode='a', index=False)
            results_file_path = '2okin_spreed3.py' 

            with open(results_file_path, 'a') as file:
                file.write(resultss + '\n')
            predictionss = model.predict(kin3_norm)[:,0] 
            ReEEE = np.array(predictionss)
            return ReEEE    
        
def rchi2_Loss7(kin, pars, F_data, F_err, phi, dvcs, ReH, ReE):  
    #print('pars') 
    #print(pars[0]) 
    kin = tf.cast(kin, pars.dtype)
    F_dnn = tf.reshape(bkm10.total_xs7(kin, pars, phi, dvcs, ReH, ReE), [-1])
    F_data = tf.cast(F_data, pars.dtype)
    F_err = tf.cast(F_err, pars.dtype) 
    loss = tf.reduce_mean(tf.square( (F_dnn - F_data) / (F_err) ) ) 
    return loss 

def fit_replica7(i, pseudoo, uniqu_cff):
    epochs = 500   
    # ----- 2 input data -----------  
    #pseduo = filter_unique_sets(pseudo)
    
    
    def build_model7():    # smae as above, but longer as it got cut off, 5000 epochs
    # model 75, info path: /media/lily/Data/GPDs/ANN/KMI/tunning/kt_RS
        #j=25
        j= 27  
        model = tf.keras.Sequential([ 
                ...
                tf.keras.layers.Dense(75 *j, activation="tanh"), tf.keras.layers.Dense(70 *j, activation="tanh"), 
                tf.keras.layers.Dense(60 *j, activation="tanh"), tf.keras.layers.Dense(51 *j, activation="tanh"),   
                tf.keras.layers.Dense(1, activation="linear") # ReH, ReE, ReHt, no dvcs dvcs 
            ])    
        return model   
...
#think I am goona keep, train and test outputs the same, so just the value on first set of phi
    ...
    def train_step(loss_inputs, inputs, targets, weights):
        for ii in range(23, -1, -1):
            ...
            with tf.GradientTape() as tape:
                pars = model(inputs) 
                loss_value = rchi2_Loss7(loss_inputs, pars, targets[ii], weights[ii], PHI[ii], dvcss, ReHss, ReEss)
            grads = tape.gradient(loss_value, model.trainable_weights)
            optimizer.apply_gradients(zip(grads, model.trainable_weights))
        return loss_value
    ...
    def test_step(loss_inputs, inputs, targets, weights):
        pars = model(inputs)
        val_loss_value = rchi2_Loss7(loss_inputs, pars, targets, weights, PHI[0], dvcsvv, ReHvv, ReEvv)
        return val_loss_value
...
###################################################################################################################################
        if (epoch==(epochs-1)):
            predictionss = model.predict(kin3_norm)
            predictions_resultss = np.array(predictionss)
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, pars_mape[0], total_mape) 
            joined_array = np.concatenate((predictions_resultss, kin3_norm), axis=1)
           
            cff_results = pd.DataFrame(joined_array, columns = ['ReHt_results', 'QQ', 'x_B', 't' ])
           
            
            cff_results.to_csv('3okin_spreed3.csv', mode='a', index=False )
            results_file_path = '3okin_spreed3.py' 
