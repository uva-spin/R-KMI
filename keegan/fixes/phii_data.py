...
class BHDVCStf(object): 
    ...
    @tf.function
    def total_xs(self, kins, cffs,phi,  dvcs):
        ffs = FFs()
        c0fit = dvcs
...
def rchi2_Loss(kin, pars, F_data, F_err, phi, dvcs):
    #print('pars')
    #print(pars[0])
    kin = tf.cast(kin, pars.dtype)
    F_dnn = tf.reshape(bkm10.total_xs(kin, pars, phi,  dvcs), [-1])
...
#think I am goona keep, train and test outputs the same, so just the value on first set of phi
    ...
    def train_step(loss_inputs, inputs, targets, weights):
        for ii in range(23, -1, -1):
            ...
            with tf.GradientTape() as tape:
                pars = model(inputs) 
                loss_value = rchi2_Loss(loss_inputs, pars, targets[ii], weights[ii], PHI[ii], dvcss)
    ...
    def test_step(loss_inputs, inputs, targets, weights):
        pars = model(inputs)
        val_loss_value = rchi2_Loss(loss_inputs, pars, targets, weights, PHI[0], dvcsv)
...
#took awya the rmse metric for the F. 
# two ides, step through phi in testing step, or in the loss function itself. 
