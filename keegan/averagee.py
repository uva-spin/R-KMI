...
class BHDVCStf(object): 
    ...
    @tf.function
    def total_xs(self, kins, cffs, dvcs):
        ffs = FFs()
        c0fit = dvcs
...
def fit_replica(i, pseudo, j):
    ...
    # model 75, info path: /media/lily/Data/GPDs/ANN/KMI/tunning/kt_RS
        ...
        model = tf.keras.Sequential([ 
                ...
                tf.keras.layers.Dense(3, activation="linear") # ReH, ReE, ReHt, no dvcs dvcs 
            ])    
        return model   
    ...
    pars_true = np.array([pseudo['ReH'], pseudo['ReE'], pseudo['ReHtilde']]).transpose()#, pseudo['dvcs']]).transpose() # true parameters
    #print(len(pars_true))
    ...
    def train_step(loss_inputs, inputs, targets, weights):
        with tf.GradientTape() as tape:
            pars = model(inputs)
            loss_value = rchi2_Loss(loss_inputs, pars, targets, weights, dvcss)
    ...
    def test_step(loss_inputs, inputs, targets, weights):
        pars = model(inputs)
        val_loss_value = rchi2_Loss(loss_inputs, pars, targets, weights, dvcsv)
    ...
    def rmseMetric(kin, kin3_norm, pars_true, F_errors):    
        ...
        F_dnn = tf.reshape(bkm10.total_xs(kin, pars, dvcs), [-1])
        F_true = tf.reshape(bkm10.total_xs(kin, pars_true, dvcs), [-1])
        weights = 1. / F_errors
    ...
    for epoch in range(epochs):
        ...
        pars_mape = [metricWrapper(m, kin3_norm, pars_true).numpy()  for m in range(3)]
...
###################################################################################################################################
        if (epoch==(epochs-1)):
            ...
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} F_rmse: {:.5f} ReH_mape: {:.5f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, F_rmse, pars_mape[0], pars_mape[1], pars_mape[2], total_mape)
           
            cff_results = pd.DataFrame(predictions_resultss, columns = ['ReH_results', 'ReE_results', 'ReHt_results'])
            
            cff_results.to_csv('1BReH_ReHt_ReE1.csv', mode='a', index=False, header=False)
            results_file_path = '1BReH_ReHt_ReE1.py'
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} F_rmse: {:.5f} ReH_mape: {:.5f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, F_rmse, pars_mape[0], pars_mape[1], pars_mape[2], total_mape)
   ...
   # history = {'loss': train_loss_results, 'val_loss': val_loss_results, 'ReH_mape': ReH_mape_results, 'ReE_mape': ReE_mape_results, 'ReHt_mape': ReHt_mape_results, 'total_mape': total_mape_results}
   # tf.keras.models.save_model(model, 'models/'+GPD_MODEL+'/test/fit_replica_'+str(i)+'.keras') # need "tf.keras.models.save_model" to save custom layer
    ...
    '''if (i<10):
        ...
        #plt.savefig('single_model_0'+str(i))
        ...
        axs[1,1].plot(ReHt_mape_results, label = 'mape')
        ...
        axs[1,1].set_ylabel("$\mathfrak{Re}\mathcal{\widetilde{H}}$_mape", fontsize = 18)
        plt.savefig('single_model_m22'+str(i))'''
...
def rchi2_Loss2(kin, pars, F_data, F_err, dvcs, ReH):
    #print('pars')
    #print(pars[0])
    kin = tf.cast(kin, pars.dtype)
    F_dnn = tf.reshape(bkm10.total_xs2(kin, pars, dvcs, ReH ), [-1])
    F_data = tf.cast(F_data, pars.dtype)
    F_err = tf.cast(F_err, pars.dtype) 
    loss = tf.reduce_mean(tf.square( (F_dnn - F_data) / (F_err) ) )
    return loss  

def fit_replica2(i, pseudo, j):
    epochs = 501
    # ----- prepare input data -----------  
    #pseduo = filter_unique_sets(pseudo)
    ...
    # model 75, info path: /media/lily/Data/GPDs/ANN/KMI/tunning/kt_RS
        ...
        model = tf.keras.Sequential([ 
                Masking(mask_value=0, input_shape=(3, )),
                tf.keras.layers.Dense(110*j, activation="sigmoid", input_shape=(3,)), 
                tf.keras.layers.Dense(105 *j, activation="tanhshrink"),tf.keras.layers.Dense(95 *j, activation="tanh"),
                tf.keras.layers.Dense(80 *j, activation="tanh"),tf.keras.layers.Dense(80 *j, activation="tanh"),
                tf.keras.layers.Dense(80 *j, activation="tanh"),tf.keras.layers.Dense(60 *j, activation="tanh"),
                tf.keras.layers.Dense(60 *j, activation="tanh"),tf.keras.layers.Dense(51 *j, activation="tanh"),
                tf.keras.layers.Dense(40 *j, activation="tanh"), tf.keras.layers.Dense(20 *j, activation="tanh"),
                tf.keras.layers.Dense(2 , activation="linear") # , ReE, ReHt, no dvcs reh 
            ])    
        return model
    ...
    QQ_norm, xB_norm, t_norm = normalize(data['QQ'] , data['xB'], data['t']) 
    kin3_norm = np.array([QQ_norm, xB_norm, t_norm]).transpose() # model inputs
    pars_true = np.array([ pseudo['ReE'], pseudo['ReHtilde']]).transpose()#, pseudo['dvcs']]).transpose() # true parameters
    #print(len(pars_true))
    dvcs = np.array([pseudo['dvcs']]).transpose() 
    ...
    def train_step(loss_inputs, inputs, targets, weights):
        with tf.GradientTape() as tape:
            pars = model(inputs)
            loss_value = rchi2_Loss2(loss_inputs, pars, targets, weights, dvcss, ReHss)
        grads = tape.gradient(loss_value, model.trainable_weights)
        optimizer.apply_gradients(zip(grads, model.trainable_weights))
        return loss_value
    ...
    def test_step(loss_inputs, inputs, targets, weights):
        pars = model(inputs)
        val_loss_value = rchi2_Loss2(loss_inputs, pars, targets, weights, dvcsv, ReHvv)
        return val_loss_value
    ...
    def rmseMetric(kin, kin3_norm, pars_true, F_errors):    
        ...
        pars_true = tf.cast(pars_true, pars.dtype)
        F_dnn = tf.reshape(bkm10.total_xs2(kin, pars, dvcs, ReH), [-1])
        F_true = tf.reshape(bkm10.total_xs2(kin, pars_true, dvcs, ReH), [-1])
        weights = 1. / F_errors
        rmse.update_state(F_true, F_dnn, sample_weight = weights)
        return tf.convert_to_tensor(rmse.result(), np.float32)
    ...
    for epoch in range(epochs):
        ...
        total_mape_results.append(total_mape)
        #ReH_mape_results.append(pars_mape[0])
        ReE_mape_results.append(pars_mape[0])
        ReHt_mape_results.append(pars_mape[1]) 
        rmse.reset_states()
        mape.reset_states()
        '''if (ReH_mape_results[-1]<20 or ReE_mape_results[-1]<20 or ReHt_mape_results[-1]<20 ):
            if epoch>501:
                predictionss = model.predict(kin3_norm)
                predictions_resultss = np.array(predictionss)
                resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} F_rmse: {:.5f} ReH_mape: {:.5f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, F_rmse, pars_mape[0], pars_mape[1], pars_mape[2], total_mape)
                ...
                cff_results.to_csv('single_ndvcs_resultsm22.csv', mode='a', index=False, header=False)
                results_file_path = 'single_ndvcs_resultsm22.py'
                resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} F_rmse: {:.5f} ReH_mape: {:.5f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, F_rmse, pars_mape[0], pars_mape[1], pars_mape[2], total_mape)
...
###################################################################################################################################
        if (epoch==(epochs-1)): 
            ...
            predictionsss = model.predict(kin3_norm)
            predictions_resultss = np.array(predictionsss)
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} F_rmse: {:.5f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, F_rmse, pars_mape[0], pars_mape[1], total_mape)
           
            cff_results = pd.DataFrame(predictions_resultss, columns = [ 'ReE_results', 'ReHt_results'])
            
            cff_results.to_csv('2BcffReH_ReHt_ReE1.csv', mode='a', index=False, header=False)
            results_file_path = '2BReH_ReHt_ReE1.py' 

            with open(results_file_path, 'a') as file:
                file.write(resultss + '\n')
            #return ReHH
            predictionss = model.predict(kin3_norm)[:,1]
            ReHtt = np.array(predictionss)
            return ReHtt
...
def rchi2_Loss3(kin, pars, F_data, F_err, dvcs, ReH, ReHt):
    #print('pars')
    #print(pars[0])
    kin = tf.cast(kin, pars.dtype)
    F_dnn = tf.reshape(bkm10.total_xs3(kin, pars, dvcs, ReH, ReHt ), [-1])
    F_data = tf.cast(F_data, pars.dtype)
    F_err = tf.cast(F_err, pars.dtype) 
    loss = tf.reduce_mean(tf.square( (F_dnn - F_data) / (F_err) ) )
    return loss          
    
def fit_replica3(i, pseudo, j):
    epochs = 351
    # ----- prepare input data -----------  
    #pseduo = filter_unique_sets(pseudo) 
    
    
    def build_model3():    # array = [35.01824, 27]
    # model 75, info path: /media/lily/Data/GPDs/ANN/KMI/tunning/kt_RS
        # j=25
        j = 4
        model = tf.keras.Sequential([ 
                ...
                tf.keras.layers.Dense(80 *j, activation="tanh"),tf.keras.layers.Dense(60 *j, activation="tanh"),
                tf.keras.layers.Dense(40 *j, activation="tanh"), tf.keras.layers.Dense(20 *j, activation="tanh"),
                tf.keras.layers.Dense(1 , activation="linear") # , ReE, ReHt, no dvcs reh 
            ])    
        return model
    ...
    QQ_norm, xB_norm, t_norm = normalize(data['QQ'] , data['xB'], data['t']) 
    kin3_norm = np.array([QQ_norm, xB_norm, t_norm]).transpose() # model inputs
    #pars_true = np.array([ pseudo['ReHtilde']]).transpose()#, pseudo['dvcs']]).transpose() # true parameters
    pars_true = np.array([ pseudo['ReE']]).transpose()#, pseudo['dvcs']]).transpose() # true parameters
    #print(len(pars_true))
    dvcs = np.array([pseudo['dvcs']]).transpose() 
    ReH = np.array([pseudo['ReH']]).transpose()
    ...
    def train_step(loss_inputs, inputs, targets, weights):
        with tf.GradientTape() as tape:
            pars = model(inputs)
            loss_value = rchi2_Loss3(loss_inputs, pars, targets, weights, dvcss, ReHss, ReHtss)
        grads = tape.gradient(loss_value, model.trainable_weights)
        optimizer.apply_gradients(zip(grads, model.trainable_weights))
        return loss_value
    ...
    def test_step(loss_inputs, inputs, targets, weights):
        pars = model(inputs)
        val_loss_value = rchi2_Loss3(loss_inputs, pars, targets, weights, dvcsv, ReHvv, ReHtvv)
        return val_loss_value
    ...
    def rmseMetric(kin, kin3_norm, pars_true, F_errors):    
        ...
        pars_true = tf.cast(pars_true, pars.dtype)
        F_dnn = tf.reshape(bkm10.total_xs3(kin, pars, dvcs, ReH, ReHt), [-1])
        F_true = tf.reshape(bkm10.total_xs3(kin, pars_true, dvcs, ReH, ReHt ), [-1])
        weights = 1. / F_errors
        rmse.update_state(F_true, F_dnn, sample_weight = weights)
        return tf.convert_to_tensor(rmse.result(), np.float32)
    ...
    for epoch in range(epochs):
        ...
        # Update metrics    
        F_rmse = rmseMetric(kin, kin3_norm, pars_true, pseudo['errF'])
        pars_mape = [metricWrapper(m, kin3_norm, pars_true).numpy()  for m in range(1 )]
        total_mape = np.mean(pars_mape) 
...
###################################################################################################################################
        if (epoch==(epochs-1)): 
            predictionss = model.predict(kin3_norm)
            predictions_resultss = np.array(predictionss)
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} F_rmse: {:.5f} ReE_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, F_rmse, pars_mape[0], total_mape)
           
            cff_results = pd.DataFrame(predictions_resultss, columns = [ 'ReE_results'])
            
            cff_results.to_csv('3BcffReH_ReHt_ReE1.csv', mode='a', index=False, header=False)
            results_file_path = '3BReH_ReHt_ReE1.py' 
...
for i in range(0, 200): 
    j=27
    start = time.time() 
    stor1 += [fit_replica(i, pseudo, j)]
#ReH_mean = np.mean(np.array(stor1), axis = 0)
copy = pseudo.copy()
copy['ReH'] = np.mean(np.array(stor1), axis = 0)
copy.to_csv('Bstore1ReH_ReHt_ReE1.csv', index=False)

stor2 = []
for i in range(0, 200):
    j=27
    newwww = 'Bstore1ReH_ReHt_ReE1.csv'
    secondd = get_data(newwww)
    pseudoo = filter_unique_sets(secondd)   
    #fit_replica2(i, pseudoo)
    stor2 += [fit_replica2(i, pseudo, j)]
    
copy = pseudo.copy()
copy['ReHt'] = np.mean(np.array(stor1), axis = 0)
copy.to_csv('Bstore2ReH_ReHt_ReE1.csv', index=False)    
    
for i in range(0, 200):
    j=27
    newwww = 'Bstore2ReH_ReHt_ReE1.csv'
    secondd = get_data(newwww)
    pseudoo = filter_unique_sets(secondd)   
    #fit_replica2(i, pseudoo)
    fit_replica3(i, pseudo, j)
