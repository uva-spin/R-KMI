...
def fit_replica(i, pseudo, j): 
    ...
    if cross_validation:
        ...
        kin_train, kin_test, kin3_norm_train, kin3_norm_test, F_train, F_test, Ferr_train, Ferr_test = train_test_split(kin, kin3_norm, data['F'], data['errF'], test_size=0.10, random_state=42) #############################################, gonna have to change this, also the reps 
...
################################################
#think I am goona keep, train and test outputs the same, so jsut the value on first set of phi
    @tf.function 
...
###################################################################################################################################
        if (epoch==(epochs-1)):
            ...
            cff_results.to_csv('phi_test.csv', mode='a', index=False, header=False) 
            results_file_path = 'phi_test.py' 
...
def get_data(neww):
    ...
    return df
stor1 = []
for i in range(0, 200):  #this changes the number of replicass
    j=27
    start = time.time() 
