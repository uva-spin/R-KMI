...
###################################################################################################################################
        if (epoch==(epochs-1)):
            ...
            cff_results.to_csv('1cffReH_ReHt_ReE11.csv', mode='a', index=False, header=False)
            results_file_path = '1cffReH_ReHt_ReE11.py'
            resultss = "Epoch {:03d}: Loss: {:.3f} val_Loss: {:.3f} F_rmse: {:.5f} ReH_mape: {:.5f} ReE_mape: {:.5f} ReHt_mape: {:.5f} total_mape: {:.5f}".format(epoch, loss_value, val_loss_value, F_rmse, pars_mape[0], pars_mape[1], pars_mape[2], total_mape)
            ...
            copy = pseudo.copy()
            copy['ReH'] = ReHH
            copy.to_csv('store1ReH_ReHt_ReE11.csv', index=False) 
...
###################################################################################################################################
        if (epoch==(epochs-1)): 
            ...
            ReHt = np.array(predictionss)
            copy = pseudo.copy()
            copy['ReHtilde'] = ReHt
            copy.to_csv('store2ReH_ReHt_ReE11.csv', index=False)
            ...
            cff_results.to_csv('2cffReH_ReHt_ReE11.csv', mode='a', index=False, header=False)
            results_file_path = '2ReH_ReHt_ReE11.py' 
...
###################################################################################################################################
        if (epoch==(epochs-1)): 
            ...
            cff_results.to_csv('3cffReH_ReHt_ReE11.csv', mode='a', index=False, header=False)
            results_file_path = '3ReH_ReHt_ReE11.py' 
...
for i in range(0, 200):
    j=27
    start = time.time() 
    fit_replica(i, pseudo, j)
    newwww = 'store1ReH_ReHt_ReE11.csv'
    secondd = get_data(newwww)
    pseudoo = filter_unique_sets(secondd)   
    #fit_replica2(i, pseudoo)
    fit_replica2(i, pseudo, j)
    newwww = 'store2ReH_ReHt_ReE11.csv'
    secondd = get_data(newwww)
    pseudoo = filter_unique_sets(secondd)   
    #fit_replica2(i, pseudoo)
