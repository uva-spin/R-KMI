...
for set in range(1, NUM_OF_SETS+1):
    ...
    ln_dif_lmi = []
    ln_dif_lmi.append('$|true - mean|$ = '+ str('%.3g' % abs(par_mean['ReH'] - par_true['ReH'])))
    ln_dif_lmi.append('$|true - mean|$ = '+ str('%.3g' % abs(par_mean['ReE'] - par_true['ReE'])))
    ln_dif_lmi.append('$|true - mean|$ = '+ str('%.3g' % abs(par_mean['ReHt'] - par_true['ReHt'])))
    ln_dif_lmi.append('$|true - mean|$ = '+ str('%.2g' % abs(par_mean['dvcs'] - par_true['dvcs'])))
    ln_dif_loc = []
    ln_dif_loc.append('$|true - mean|$ = '+ str('%.3g' % abs(par_mean_local['ReH'] - par_true['ReH'])))
    ln_dif_loc.append('$|true - mean|$ = '+ str('%.3g' % abs(par_mean_local['ReE'] - par_true['ReE'])))
    ln_dif_loc.append('$|true - mean|$ = '+ str('%.3g' % abs(par_mean_local['ReHt'] - par_true['ReHt'])))
    ln_dif_loc.append('$|true - mean|$ = '+ str('%.2g' % abs(par_mean_local['dvcs'] - par_true['dvcs'])))
    ...
    axs[0,0].legend([ln_true[0], lmi_box, ln_mean_lmi[0], extra, extra, loc_box, ln_mean_loc[0], extra, extra],[ln_true[0].get_label(), lmi_tx, ln_mean_lmi[0].get_label(), ln_std_lmi[0], ln_dif_lmi[0], loc_tx, ln_mean_loc[0].get_label(), ln_std_loc[0], ln_dif_loc[0]], fontsize = 18, loc ="best")
    axs[0,1].set_xlabel("$\mathfrak{Re}\mathcal{E}$", fontsize = 18)
    axs[0,1].set_ylabel("$frequency$", fontsize = 18)
    axs[0,1].legend([ln_true[1], lmi_box, ln_mean_lmi[1], extra, extra, loc_box, ln_mean_loc[1], extra, extra],[ln_true[1].get_label(), lmi_tx, ln_mean_lmi[1].get_label(), ln_std_lmi[1], ln_dif_lmi[1], loc_tx, ln_mean_loc[1].get_label(), ln_std_loc[1], ln_dif_loc[1]], fontsize = 18, loc ="best")
    axs[1,0].set_xlabel("$\mathfrak{Re}\mathcal{\widetilde{H}}$", fontsize = 18)
    axs[1,0].set_ylabel("$frequency$", fontsize = 18)
    axs[1,0].legend([ln_true[2], lmi_box, ln_mean_lmi[2], extra, extra, loc_box, ln_mean_loc[2], extra, extra],[ln_true[2].get_label(), lmi_tx, ln_mean_lmi[2].get_label(), ln_std_lmi[2], ln_dif_lmi[2], loc_tx, ln_mean_loc[2].get_label(), ln_std_loc[2], ln_dif_loc[2]], fontsize = 18, loc ="best")
    axs[1,1].set_xlabel("$dvcs$", fontsize = 18)
    axs[1,1].set_ylabel("$frequency$", fontsize = 18)
    # axs[1,1].set_xlim(left=0)
    axs[1,1].set_xlim(-0.015, 0.05)
    axs[1,1].legend([ln_true[3], lmi_box, ln_mean_lmi[3], extra, extra, loc_box, ln_mean_loc[3], extra, extra],[ln_true[3].get_label(), lmi_tx, ln_mean_lmi[3].get_label(), ln_std_lmi[3], ln_dif_lmi[3], loc_tx, ln_mean_loc[3].get_label(), ln_std_loc[3], ln_dif_loc[3]], fontsize = 18, loc ="best")
