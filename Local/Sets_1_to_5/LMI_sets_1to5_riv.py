...
def fit_replica(i, pseudo):
    ...
    models = build_model()
    models.summary()
    history = models.fit(kin_train, F_train, epochs=100, batch_size=10, validation_data=(kin_test, F_test))
    tf.keras.models.save_model(models, 'models/'+GPD_MODEL+'/fit_replica_'+str(i)+'.keras') # need "tf.keras.models.save_model" to save custom layer
    np.save('models/'+GPD_MODEL+'/history_fit_replica_'+str(i)+'.npy',history.history) 
